...
> ✅ **状态：M1–M4 已实现**。M1 只读体检 → 交互式 HTML 报告（CC+Codex 的 skills/MCP，含 plugin 自带项）；M2 `apply` 处置层（dry-run 默认、`--apply` 带备份）+ hooks/memory 维度；M3 `serve` 本地服务一键执行（127.0.0.1 + token + 确认 + 备份）；M4 memory 跨文件重复检测、`digest`（缓存秒回）+ `install-hook`（SessionStart 巡检）、Codex `config.toml` 安全删除 MCP。**73 个测试全绿**，真机全程零误伤。仓库：https://github.com/sterlingdon/donald-agent-hygiene
...
# 预览清理（dry-run，绝不改动）→ 加 --apply 才执行（带备份）
cd scripts && python -m hygiene.cli apply \
  --home "$HOME" --codex-home "$HOME/.codex" \
  --projects "$HOME/.claude/projects" --sessions "$HOME/.codex/sessions" \
  --cwd "$PWD" --severity green                # 预览
# … 同样命令末尾加 --apply 执行

# 起本地服务，在浏览器里逐项「一键执行」（127.0.0.1 + token + 二次确认 + 备份）
cd scripts && python -m hygiene.cli serve \
  --home "$HOME" --codex-home "$HOME/.codex" \
  --projects "$HOME/.claude/projects" --sessions "$HOME/.codex/sessions" \
  --cwd "$PWD" --port 8765

# 一行健康摘要（读缓存，秒回，可挂 SessionStart hook）
cd scripts && python -m hygiene.cli digest
cd scripts && python -m hygiene.cli install-hook --home "$HOME"          # 预览安装巡检 hook
cd scripts && python -m hygiene.cli install-hook --home "$HOME" --apply  # 真正安装（备份 settings.json）
...
执行清理（归档 skill / 删 user MCP / 禁用插件）**默认 dry-run，改写前自动备份**到 `~/.agent-hygiene-backups/`；
Codex `config.toml` 永不自动改写（只出 `codex mcp remove` 命令）；hooks/memory 永不自动清理。
