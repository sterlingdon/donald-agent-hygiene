...
def test_builtins_excluded_from_skills(tmp_path):
    d = tmp_path / "sessions" / "2026" / "06" / "08"; d.mkdir(parents=True)
    lines = [
        _ev("function_call", name="exec_command"),
        _ev("function_call", name="browser_navigate"),
        _ev("function_call", name="my-real-skill"),
    ]
    (d / "rollout-x.jsonl").write_text("\n".join(json.dumps(x) for x in lines))
    skills, _ = collect_usage(str(tmp_path / "sessions"))
    assert "exec_command" not in skills and "browser_navigate" not in skills
    assert skills["my-real-skill"]["count"] == 1
